...
export default function ProductSkeleton({ count = 6, className, viewMode = 'grid' }: ProductSkeletonProps) {
    ...
    <div className={`${gridLayoutClass} ${className || ''} gap-0 border-t border-l border-gray-200 dark:border-gray-700`} data-testid="product-skeleton">
      {[...Array(count)].map((_, index) => (
        <div key={index} className="border-r border-b border-gray-200 dark:border-gray-700" data-testid={`skeleton-cell-${index}`}>
          <div className="block w-full overflow-hidden p-4 h-full bg-white dark:bg-gray-800 rounded-lg shadow-md animate-pulse">
            <div className={`flex h-full ${viewMode === 'horizontal' ? 'flex-row gap-x-6 gap-y-8 min-h-[300px]' : 'flex-col'}`}>
              <div className={`relative ${viewMode === 'horizontal' ? 'w-2/5' : 'w-full'} ${viewMode === 'horizontal' ? 'aspect-2/3' : 'aspect-square'} overflow-hidden rounded-lg bg-gray-300 dark:bg-gray-600`}></div>
